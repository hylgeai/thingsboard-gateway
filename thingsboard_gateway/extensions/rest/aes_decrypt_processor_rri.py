...
class AESDecryptConverterRRI(Converter):
    ...
    def convert(self, config, data):
        ...
        try:
            ...
            # 提取设备信息 (只取第一个设备，如有多个)
            device_info_list = telemetry_data.get("deviceinformation", [])
            if not device_info_list:
                self._log.warning("未找到设备信息")
                return converted_data

            # 获取第一个设备的UID
            device_uid = device_info_list[0].get("uid", "")
            if not device_uid:
                self._log.warning("未找到设备UID")
                return converted_data

            # 提取情感数据
            emotion_list = telemetry_data.get("emotion", [])
            if not emotion_list:
                self._log.warning("未找到情感数据")
                return converted_data

            # 处理每条情感数据
            for emotion_data in emotion_list:
                # 提取rri和time
                rri = emotion_data.get("rri", "")
                timestamp = emotion_data.get("time", "")
            ...
            self._log.debug(f"成功转换数据: 设备={device_uid}, 情感数据点={len(emotion_list)}")
